...
```{r N_data}
if (scale == "year"){
  real_pop <- readRDS(here::here("./analysis/data/Nplot_data.RDS"))  
} else if (scale == "month"){
...
What was the rate of growth during this period?

```{r growthrateN}
slope_data <- real_pop %>% 
  ## Use 1st May b/c we calculated over whole months
  ## Numbers are similar.
  filter(date %in% as.Date(c("1996-05-01", "2012-04-01")))

(max(slope_data$pop_size) - min(slope_data$pop_size))/16

## If we take numbers at exact start date
(max(slope_data$pop_size) - (119+65))/16
```

And if we tried to calculate the slope w/ CI for this growth period?

```{r growthNmodel}
mod_data <- data.frame(date = as.Date("1996-04-01"),
                       pop_size = fetch_pop_number(main.clans = TRUE, at = find_pop_date.observation.first())) %>% 
  bind_rows(real_pop) %>% 
  filter(date %in% seq.Date(as.Date("1996-04-01"),
                            as.Date("2012-04-01"), "6 month")) %>% 
  mutate(n = 1:n())

mod <- spaMM::fitme(pop_size ~ n + AR1(1|n), data = mod_data)
...
```{r DHARMaN}
library(DHARMa)
simresid <- simulateResiduals(mod, n = 1000)

testUniformity(simresid)
```
...
How can we describe the period after 2012?

```{r}
post2012_data <- real_pop %>% 
  ## Use 1st May b/c we calculated over whole months
  ## Numbers are similar.
  filter(date > as.Date("2012-04-01"))

median(post2012_data$pop_size)

range(post2012_data$pop_size)
```

Minimum size during last outbreak.

```{r}
post2012_data %>% 
  filter(pop_size == min(pop_size))
```
...
```{r}
K_model1 <- fitme(globalK ~ year, data = poisson_data, family = "poisson")
```
...
K_model2 <- fitme(globalK ~ year, data = poisson_data, family = "gaussian")
```
...
```{r}
K_model3 <- fitme(globalK ~ year, data = poisson_data, family = spaMM::negbin2())
```
...
```{r}
global_mod <- fitme(globalK ~ year + 1, data = poisson_data, family = spaMM::negbin2())
null_mod   <- fitme(globalK ~ 1, data = poisson_data, family = spaMM::negbin2())

LRT_result_poisson <- LRT(global_mod, null_mod, boot.repl = 999, nb_cores = 10, seed = 123)
data.frame(basic = LRT_result_poisson$basicLRT$p_value,
           bartlett = LRT_result_poisson$BartBootLRT$p_value,
           bootstrap = LRT_result_poisson$rawBootLRT$p_value)
```
...
```{r ktc_data}
if (file.exists(here::here("./analysis/data/Kplot_clan_data.RDS"))) {
  ...
  saveRDS(Kplot_data, file = here("./analysis/data/Kplot_clan_data.RDS"))
}
...
```{r}
K_model_growthperiod <- fitme(globalK ~ year, data = poisson_data %>% filter(year <= 2012), family = "gaussian")
null_mod <- fitme(globalK ~ 1, data = poisson_data %>% filter(year <= 2012), family = "gaussian")
...
K_model_perclan_growthperiod <- fitme(median ~ year:name + name + AR1(1|n),
                                      data = plot_data %>% filter(year <= 2012), family = "gaussian")
...
null_mod   <- fitme(cbind(ad_male, ad_fem) ~ AR1(1|n), data = ratio_data, family = "binomial")
...
```{r}
spaMM::confint.HLfit(global_mod, parm = "n", boot_args = list(nsim = 999, nb.cores = 10, seed = 123))
```
...
null_mod   <- fitme(cbind(young, ad_male + ad_fem) ~ AR1(1|n), data = ratio_data, family = "binomial")
