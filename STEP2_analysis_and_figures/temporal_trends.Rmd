...
if (!file.exists(here::here("./data/Kplot_data.RDS"))) {
  ...
  Kplot_data <- purrr::map_df(.x = years,
                              .f = function(year){
                                ...
                                txt_files <- list.files(here(paste0("./STEP1_estimateK/", year)),
...
N_model <- fitme(pop_size ~ year + AR1(1|n), data = real_pop, family = "poisson")
N_model_null <- fitme(pop_size ~ 1 + AR1(1|n), data = real_pop, family = "poisson")
...
real_pop_growth <- real_pop |> 
  filter(year <= growth_period_until)

N_model <- fitme(pop_size ~ year + AR1(1|n), data = real_pop_growth, family = "poisson")
N_model_null <- fitme(pop_size ~ 1 + AR1(1|n), data = real_pop_growth, family = "poisson")
...
```{r}
real_pop_aftergrowth <- real_pop |> 
  filter(year > growth_period_until)

N_model <- fitme(pop_size ~ year + AR1(1|n), data = real_pop_aftergrowth, family = "poisson")
N_model_null <- fitme(pop_size ~ 1 + AR1(1|n), data = real_pop_aftergrowth, family = "poisson")
...
```{r}
global_mod <- fitme(globalK ~ year, data = poisson_data, family = spaMM::negbin2())
...
if (file.exists(here::here("./data/Kplot_clan_data.RDS"))) {
  ...
  Kplot_data <- purrr::map_df(.x = years,
                              .f = function(year, folder_name){
                                ...
                                txt_files <- list.files(here(paste0("~/data/simulation_results/analysis/STEP1_estimateK/", year)),
                                                        pattern = ".txt", full.names = TRUE)
...
K_model_perclan_gaussian <- fitme(median ~ year:name + name + AR1(1|n), data = plot_data, family = "poisson")
...
```{r LRT_Ktc}
global_mod <- fitme(median ~ year:name + name + AR1(1|n), data = plot_data, family = "poisson")
null_mod   <- fitme(median ~ name + AR1(1|n), data = plot_data, family = "poisson")
```
...
```{r}
K_model_growthperiod <- fitme(globalK ~ year + AR1(1|n), data = poisson_data %>% filter(year <= growth_period_until), family = "poisson")
null_mod <- fitme(globalK ~ 1 + AR1(1|n), data = poisson_data %>% filter(year <= growth_period_until), family = "poisson")
summary(K_model_growthperiod)
```
```{r}
## Asympototic for consistency
LRT(K_model_growthperiod, null_mod)
# LRT_result_gaussian <- LRT(K_model_growthperiod, null_mod, boot.repl = 999, nb_cores = 10, seed = 123)
# data.frame(basic = LRT_result_gaussian$basicLRT$p_value, bartlett = LRT_result_gaussian$BartBootLRT$p_value, bootstrap = LRT_result_gaussian$rawBootLRT$p_value)
```
...
K_model_perclan_growthperiod <- fitme(median ~ year:name + name + AR1(1|n),
                                      data = plot_data %>% filter(year <= growth_period_until), family = "poisson")
...
# LRT_result_gaussian <- LRT(K_model_perclan_growthperiod, null_mod, boot.repl = 999, nb_cores = 10, seed = 123)
# data.frame(basic = LRT_result_gaussian$basicLRT$p_value, bartlett = LRT_result_gaussian$BartBootLRT$p_value, bootstrap = LRT_result_gaussian$rawBootLRT$p_value)
```
