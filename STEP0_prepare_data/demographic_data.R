...
if (file.exists(here::here("./analysis/data/Nplot_data_separate_1month.RDS"))) {
  ...
  real_pop_separate <- dplyr::tibble(date = seq(as.Date("1996-05-01"), as.Date("2022-01-01"), by = "1 month")) %>% 
  ...
  saveRDS(real_pop_separate, file = here::here("./analysis/data/Nplot_data_separate_1month.RDS"))
}

## Calculate annual
if (file.exists(here::here("./analysis/data/Nplot_data_separate_year.RDS"))) {
  ...
  saveRDS(real_pop_separate, file = here::here("./analysis/data/Nplot_data_separate_year.RDS"))
}

## Population abundance over time (Nt)
if (!file.exists(here::here("./analysis/data/Nplot_data_month.RDS"))) {
  ## EXTRACT REAL POP DATA OVER TIME
  hyenaR::load_package_database.full(db.path)
  
  real_pop <- dplyr::tibble(date = seq(as.Date("1996-05-01"), as.Date("2022-01-01"), by = "1 month")) %>% 
    dplyr::mutate(pop_size = hyenaR::fetch_pop_number(main.clans = TRUE, at = .data$date,
                                                      CPUcores = 75, .parallel.min = 285))
  saveRDS(real_pop, here::here("./analysis/data/Nplot_data_month.RDS"))
} else {
  real_pop <- readRDS(here::here("./analysis/data/Nplot_data_month.RDS"))
}

if (!file.exists(here::here("./analysis/data/Nplot_data_year.RDS"))) {
  ## EXTRACT REAL POP DATA OVER TIME
  hyenaR::load_package_database.full(db.path)
  ...
  saveRDS(real_pop, here::here("./analysis/data/Nplot_data_year.RDS"))
} else {
