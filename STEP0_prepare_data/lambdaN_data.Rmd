...
```{r}
pop_size <- data.frame(from = seq(as.Date("1996-07-01"), as.Date("2022-01-01"), by = paste(time_step, "months"))) %>%
  mutate(to = lead(from)) %>% 
  filter(!is.na(to)) %>% 
...
```{r}
readr::write_excel_csv(pop_size, here("./analysis/data/supp_data1_alternativeK_data.csv"))
```
