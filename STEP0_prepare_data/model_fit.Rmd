...
Identify best models used for the simulation. This will involve:

1. Comparing different link functions.
2. Consider different density dependent relationships

We will fit models using random intercept of year AND 6 months. We only consider time periods that have been fully monitored (i.e. we exclude 1996 when using year and first half 1996 when using 6 month time steps).
...
if (file.exists(here::here("./analysis/data/model_data.RDS"))) {
  ...
  db.path <- "~/data/hyena_data/Fisidata_2023_03_07.sqlite"
  ...
  saveRDS(model_data, here::here("./analysis/data/model_data.RDS"))
...
```{r}
exp_profiling <- function(exp, data, model){
  ...
  return(ggplot(data = plot_data) +
           ...
           labs(title = paste("Difference between exp=1 and best exponent", round(algo_loglik_optim$maximum, 2), ":\n", round(pval, digits = 2), sep = " ")) +
           theme_bw())
...
```{r}
F_surv_mod_yr <- compare_linkfn(surv ~ poly(age, 4)*rank_category2 +
                                  clan_size +
...
*No significant difference from 0 exponent* Best is 0.45

#### Plot

```{r}
best1y  <- fit_exp(x = 0.45, model = F_surv_mod_yr$logit, data = model_data$F_surv_data, verbose = FALSE)
...
```{r}
output <- exp_profiling(exp = c(-1, -0.5, -0.1, 0.1, 0.5, 1, 2, 3, 4, 5, 6), data = model_data$PreM_surv_data, model = PreM_surv_mod_yr$cloglog)
output
```
No sig difference. 2.38 is best.

#### Plot

```{r}
best1y  <- fit_exp(x = 2.38, model = PreM_surv_mod_yr$cloglog, data = model_data$PreM_surv_data, verbose = FALSE)
...
plot_data <- expand.grid(start_clan = "A", clan_size = 0:200, after1y_effort_all = 1,
  ...
  mutate(pred1y = as.numeric(predict(PreM_surv_mod_yr$logit, newdata = ., type = "response", re.form = NA)),
         pred1y_best = as.numeric(predict(best1y, newdata = ., type = "response", re.form = NA)),
         pred1y_cloglog = as.numeric(predict(PreM_surv_mod_yr$cloglog, newdata = ., type = "response", re.form = NA))) %>% 
  tidyr::pivot_longer(starts_with("pred"))
...
output <- exp_profiling(c(-2, -1, -0.5, -0.1, 0.1, 0.5, 1, 2, 3, 4, 5, 6, 7), data = model_data$PostM_surv_data,
                        model = PostM_surv_mod_yr$cloglog)
output
```

Not significantly different to 0. Best is 0.67

#### Plot

```{r}
best1y  <- fit_exp(x = 0.67, model = PostM_surv_mod_yr$cloglog, data = model_data$PostM_surv_data, verbose = FALSE)
...
cloglog with no exponent.

### Primiparous reproduction

```{r}
F_repro_primi_mod_yr <- compare_linkfn(repro ~ months*rank_category2 +
...
```{r}
output <- exp_profiling(c(-2, -1, -0.5, -0.1, 0.1, 0.5, 1, 2, 3, 4, 5, 6, 7), data = model_data$F_repro_primi, model = F_repro_primi_mod_yr$probit)
output
```
...
```{r}
best1y  <- fit_exp(x = -0.5, model = F_repro_primi_mod_yr$probit, data = F_repro_primi_yr, verbose = FALSE)
...
Probit with no exponent is best.

### Multiparous reproduction

```{r}
F_repro_nonprimi_mod_yr <- compare_linkfn(repro ~ months*rank_category2 + months*age +
                                            clan_size*rank_category2 + after1y_effort_all + start_clan +
                                            (1|start_clan:year),
...
```{r}
output <- exp_profiling(c(-2, -1.5, -1, -0.5, -0.1, 0.1, 0.5, 1, 2, 3, 4, 5, 6, 7), data = model_data$F_repro_nonprimi, model = F_repro_nonprimi_mod_yr$probit)
output
```
...
```{r}
best1y  <- fit_exp(x = -1, model = F_repro_nonprimi_mod_yr$probit, data = model_data$F_repro_nonprimi, verbose = FALSE)
...
best1y  <- fit_exp(x = -1, model = twin_mod_yr$logit, data = twin_yr, verbose = FALSE)
