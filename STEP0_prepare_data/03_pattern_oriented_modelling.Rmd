...
```{r}
## Run simulation for 350 timesteps (i.e. ~from mid 1996 - early 2025)
POM_sim <- simulation_fitrun(number_steps = 350, models = modlist,
                             return = TRUE, name = "POM",
                             save_dir = here::here("./data/POM"),
...
for (file in all_tbls){
  ...
  write.csv(focal_tbl,
            file = here::here(paste0("./data/POM/", save_name, ".csv")), row.names = FALSE)
  
}

build_package_database.full(db.name = "POM_sim",
                            input.folder = here::here("./data/POM/"))
```

```{r}
# load_package_database.sim(POM_sim)
## We might get a warning because the sim goes slightly beyond the current date
load_package_database.full(here::here("./data/POM/POM_sim.sqlite"))
```
...
ggsave(filename = here::here("./STEP0_prepare_data/03_pattern_oriented_modelling/plots/lifespan.png"),
       width = 9, heigh = 5)
```
...
ggsave(filename = here::here("./plots/POM/LRS.png"),
       width = 9, heigh = 5)
```
...
ggsave(filename = here::here("./plots/POM/age_at_first_repro.png"),
       width = 9, heigh = 5)
```
...
ggsave(filename = here::here("./plots/POM/IBI.png"),
       width = 9, heigh = 5)
```
...
ggsave(filename = here::here("./plots/POM/rank_effect.png"),
       width = 9, heigh = 5)
```
