...
end_yr   <- 2021 # Technically we can use data from early 2022, but not enough samples for year RE
...
system.time({preMsurv_mixmod <- spaMM::fitme(surv ~ poly(age, 2)*rank_category2 +
                                               ...
                                               clan_size:rank_category2 + start_clan + (1|start_clan:year),
                                             data = PreM_surv_data,
                                             family = binomial(link = "cloglog"), method = "PQL/L")})
...
saveRDS(modlist, here::here("./analysis/data/model_list.RDS"))

# Take the start pop 1996 and increase it to different sizes
## GENERATED IN STEP0_prepare_data/starting_population.R
start_pop <- read.csv(here::here("./analysis/data/starting_data.csv"))
...
system.time({v050_K <- simulation_iterate(start_pops = start_pop,
                                          ...
                                          save_dir = "./analysis/STEP1_estimate_K",
                                          save_size = 60,
                                          iterator_seed = 123,
...
system.time({simulation_iterate(start_pops = start_pop,
                                ...
                                number_steps = 1200,
                                step_size = 1, models = modlist,
                                save_dir = "./analysis/STEP1_estimate_K/marginal",
                                save_size = 60,
                                iterator_seed = 123,
