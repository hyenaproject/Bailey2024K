...
We will use mean square error to estimate K using different traditional methods.
...
```{r}
pop_size <- read.csv(here("./analysis/data/supp_data1_alternativeK_data.csv"))
```
...
```{r}
sheph_mod <- FSA::srFuns(type = "Shepherd")
...
ggsave(output, filename = here::here("analysis/plots/alternative_K_method_new.png"),
       width = 8.25, height = 3.75, dpi = 600)
```

## PLOT 2: Show estimated values for top models

```{r}
plot_data2 <- all_fitted_data %>% 
  tidyr::unnest(cols = c(x, y))
```

```{r}
#Ricker models
plot_data_Ricker <- plot_data2 %>% 
  filter(grepl(type, pattern = "^Ricker")) %>% 
  #Filter data to just around the focal point ~500
  filter(x > 450 & x < 550)

ggplot() +
  geom_hline(yintercept = 0, lty = 2, linewidth = 0.25) +
  geom_line(data = plot_data_Ricker, aes(x = x, y = y, group = opt)) +
...
plot_data_BH <- plot_data2 %>% 
  filter(grepl(type, pattern = "^BH"))

ggplot() +
  geom_abline(slope = 1, intercept = 0, lty = 2, linewidth = 0.25) +
  geom_line(data = plot_data_BH, aes(x = x, y = y, group = opt)) +
  facet_wrap(facets = ~type) +
